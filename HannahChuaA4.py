...
#1.    Load csv data file into the program memory.
#2.    Allow the user to perform the following with menu selection. (1 mark)
#a.    Insert an item from the Inventory System (3 mark)
#b.    Delete an item from the Inventory System (1 mark)
#c.    Save the updated Inventory information back to the data csv file. (1 mark)
import csv
...
s = "===================================================="
def read_data_into_lists():
    with open("codes.csv") as codesfile:
        reader=csv.DictReader(codesfile)
        for row in reader:
...
def write_final():#saves final lists of SKU and PRODUCT_ID into data.csv
    with open("data.csv", 'w', newline='') as csvfile:
        fieldnames = ['ROW','SKU', 'PRODUCT ID','NAME','CATEGORY','SUB-CATEGORY','STOCK LEVEL']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        i=0 #i++
        while i<len(SKU):#iterate through list; loop to go to each index and saves
